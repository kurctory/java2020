...
class SolverTest {
    ...
    void testCalculateRoots() {
        ...
        b = 0;
        c = 1;
        Assertions.assertEquals("Нет решений(a = 0,b = 0,c != 0)", Solver.calculateRoots(a, b, c));
        c = 0;
        Assertions.assertEquals("x - любое число", Solver.calculateRoots(a, b, c));
        b = 1;
        c = 1;
        x = -(b / c);
        Assertions.assertEquals(String.format("%f\n", x), Solver.calculateRoots(a, b, c));
        a = 1;
        b = 0;
        c = 1;
